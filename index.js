...
//KASIH NUYFAA FXSX FDL/TYPES
//JIKA INGIN RECODE JANGAN LUPA KASIH CREDIT YA KAK:) 
const {
   WAConnection,
   MessageType,
   Presence,
...
const faa = new WAConnection()
   faa.on('qr', qr => {
   qrcode.generate(qr, { small: true })
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('QR code is ready, Scan now..','white'),color('NuyFaaツ','red'),color('X','FxSx'),color('FdlXploit','red'))
})

faa.on('credentials-updated', () => {
	const authInfo = faa.base64EncodedAuthInfo()
   console.log(`credentials updated!`)
   fs.writeFileSync('./session.json', JSON.stringify(authInfo, null, '\t'))
})
fs.existsSync('./session.json') && faa.loadAuthInfo('./session.json')
faa.connect();


faa.on('group-participants-update', async (anu) => {
		if (!welkom.includes(anu.jid)) return
		try {
			const mdata = await faa.groupMetadata(anu.jid)
			console.log(anu)
			if (anu.action == 'add') {
				num = anu.participants[0]
				try {
					ppimg = await faa.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				teks = `*Hallo* @${num.split('@')[0]}\nSelamat datang di group *${mdata.subject}*\nJangan rusuh ya\nJangan lupa intro @${num.split('@')[0]} 🗣`
				let buff = await getBuffer(ppimg)
				faa.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
				} else if (anu.action == 'remove') {
				num = anu.participants[0]
				try {
					ppimg = await faa.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				teks = `*anak haram yang keluar:v* @${num.split('@')[0]}\n*tuman kalo di masukin lagi:v*`
				let buff = await getBuffer(ppimg)
				faa.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	faa.on('message-new', async (mek) => {
		try {
			...
			const q = args.join(' ')
			const botNumber = faa.user.jid
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			pushname = faa.contacts[sender] != undefined ? faa.contacts[sender].vname || faa.contacts[sender].notify : undefined
			const groupMetadata = isGroup ? await faa.groupMetadata(from) : ''
			...
			const reply = (teks) => {
				faa.sendMessage(from, teks, text, {quoted:mek})
			}
			const sendMess = (hehe, teks) => {
				faa.sendMessage(hehe, teks, text)
			}
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? faa.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : faa.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
			const sendImage = (teks) => {
		    faa.sendMessage(from, teks, image, {quoted:mek})
		    }
		    const costum = (pesan, tipe, target, target2) => {
			faa.sendMessage(from, pesan, tipe, {quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` }}})
			}
		    const sendPtt = (teks) => {
		    faa.sendMessage(from, audio, mp3, {quoted:mek})
		    }
        ...
        if (levelRole <= 2) {
          ...
          const checkLimit = (sender) => {
          	let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return faa.sendMessage(from,`Limit request anda sudah habis\n\n_Note : limit bisa di dapatkan dengan cara ${prefix}buylimit dan dengan naik level_`, text,{ quoted: mek})
                            faa.sendMessage(from, ind.limitcount(limitCounts), text, { quoted : mek})
                            found = true
                        }
                    }
                    if (found === false) {
           ...
           const isLimit = (sender) =>{ 
              ...
              if (limits >= limitawal ) {
              	  position = true
                    faa.sendMessage(from, ind.limitend(pushname), text, {quoted: mek})
                    return true
              } else {
              	_limit
                  position = true
           ...
           return false
		    ...
		    if (isGroupAdmins) return reply('karena kamu adalah admin group, bot tidak akan kick kamu')
		      faa.updatePresence(from, Presence.composing)
		    if (messagesLink.includes("#izinadmin")) return reply('#izinadmin diterima')
		    var kic = `${sender.split('@')[0]}@s.whatsapp.net`
	    	reply(`Link Group Terdeteksi maaf ${sender.split('@')[0]} anda akan di kick dari group 5detik lagi`)
		    setTimeout( () => {
			  faa.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		    }, 3000)
		    setTimeout( () => {
			  faa.updatePresence(from, Presence.composing)
			reply("1detik")
       ...
       if (bad.includes(messagesLink)) {
			...
			switch(command) {
				case 'nobadword':  // Case By NuyFaa FxSx FdlXploit
				...
				case 'addbadword':  // Case By NuyFaa FxSx FdlXploit
                    ...
                    break
                case 'delbadword':  // Case By NuyFaa FxSx FdlXploit
                    if (!isOwner) return reply(ind.ownerb())
                    if (!isGroupAdmins) return reply(ind.admin())
                    if (args.length < 1) return reply( `Kirim perintah ${prefix}addbadword [kata kasar]. contoh ${prefix}addbadword bego`)
                    ...
                    break 
                case 'listbadword':  // Case By NuyFaa FxSx FdlXploit
                    ...
                    break 
				case 'getsticker':
				case 'gets':  // Case By NuyFaa FxSx FdlXploit
				
					namastc = body.slice(12)
					result = fs.readFileSync(`./strg/sticker/${namastc}.webp`)
					faa.sendMessage(from, result, sticker, {quoted :mek})
					break
				case 'stickerlist':
				case 'liststicker':  // Case By NuyFaa FxSx FdlXploit
					...
					faa.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": setiker } })
					break
				case 'addsticker':  // Case By NuyFaa FxSx FdlXploit
				
					if (!isQuotedSticker) return reply('Reply stiker nya')
					svst = body.slice(12)
					if (!svst) return reply('Nama sticker nya apa?')
					boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
					delb = await faa.downloadMediaMessage(boij)
					setiker.push(`${svst}`)
					fs.writeFileSync(`./strg/sticker/${svst}.webp`, delb)
					fs.writeFileSync(`./strg/stik.json`, JSON.stringify(setiker))
					faa.sendMessage(from, `Sukses Menambahkan Sticker\nCek dengan cara ${prefix}liststicker`, MessageType.text, { quoted: mek })
					break
				case 'addvn':  // Case By NuyFaa FxSx FdlXploit
					...
					faa.sendMessage(from, `Sukses Menambahkan Video\nCek dengan cara ${prefix}listvn`, MessageType.text, { quoted: mek })
					break
				case 'getvn':  // Case By NuyFaa FxSx FdlXploit
				
					namastc = body.slice(7)
					buffer = fs.readFileSync(`./strg/audio/${namastc}.mp3`)
					faa.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', quoted: mek, ptt: true })
					break
				case 'listvn':
				case 'vnlist':  // Case By NuyFaa FxSx FdlXploit
					...
					faa.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": audionye } })
					break
				case 'addimage':  // Case By NuyFaa FxSx FdlXploit
					...
					break
				case 'getimage':  // Case By NuyFaa FxSx FdlXploit
				
					namastc = body.slice(10)
					buffer = fs.readFileSync(`./strg/image/${namastc}.jpeg`)
					faa.sendMessage(from, buffer, image, { quoted: mek, caption: `Result From Database : ${namastc}.jpeg` })
					break
				case 'imagelist':
				case 'listimage':  // Case By NuyFaa FxSx FdlXploit
					...
					faa.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagenye } })
					break
				case 'addvideo':  // Case By NuyFaa FxSx FdlXploit
					...
					break
				case 'getvideo':  // Case By NuyFaa FxSx FdlXploit
				
					namastc = body.slice(10)
					buffer = fs.readFileSync(`./strg/video/${namastc}.mp4`)
					faa.sendMessage(from, buffer, video, { mimetype: 'video/mp4', quoted: mek })
					break
				case 'listvideo':
				case 'videolist':  // Case By NuyFaa FxSx FdlXploit
					...
					faa.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": videonye } })
					break	
			
				
		case 'mutual':// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				break
		case 'next':// Fix Bug Makanya Jagan Nganti Nama NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
				if (isGroup) return  reply( 'Command ini tidak bisa digunakan di dalam grup!')
				anug = getRegisteredRandomId(_registered).replace('@s.whatsapp.net','')
				await reply('Find for a partner...')
				...
				break
		case 'transfer':
				// Fix Bug Makanya Jagan Nganti Nama NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
				if (!q.includes('|')) return  reply(ind.wrongf())
                		const tujuan = q.substring(0, q.indexOf('|') - 1)
                		const jumblah = q.substring(q.lastIndexOf('|') + 1)
                		if(isNaN(jumblah)) return await reply('jumlah harus berupa angka!!')
                		if (jumblah < 500 ) return reply(`minimal transfer 500`)
				...
				faa.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": adm}})
				break
                case 'premium':
				if (!isOwner) return reply(ind.ownerb())
				premm = body.slice(9)
		...
		case 'unpremium':
				...
				faa.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": prem}})
				break
                case 'bann':
       ...
       case 'ban':
				...
				ban.push(`${bnnd}@s.whatsapp.net`)
				fs.writeFileSync('./database/pengguna/banned.json', JSON.stringify(ban))
				reply(`Berhasil membanned nomor : wa.me/${bnnd}\nAkhirnya Owner Manca Kasih Perintah Ban\nMakasih NuyFaaツ`)
				break
		case 'unban':
				if (!isOwner) return reply(ind.ownerb())
				bnnd = body.slice(7)
				...
				faa.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": ban}})
				break
		...
		case 'giftlimit': 
				...
				faa.sendMessage(from, buffer, image, {quoted: mek, caption: `${teks}`})
				await limitAdd(sender)
				break
		case 'happymod':
				// Fix Bug Makanya Jagan Nganti Nama NuyFaa
		...
		case 'cuttly':
				...
				case 'pornhub':
					...
					var porn = gh.split("/")[0];
					var hub = gh.split("/")[1];
					if (args.length < 1) return reply(`ã€Œâ—ã€Contoh : ${prefix}pornhub Nuy/Hub`)
					reply(ind.wait())
					alan = await getBuffer(`https://vinz.zeks.xyz/api/pornhub?text1=${porn}&text2=${hub}`)
					faa.sendMessage(from, alan, image, {quoted: mek})
					await limitAdd(sender)
				break
				case 'googletext':
					...
					if (args.length < 1) return reply(`ã€Œâ—ã€Contoh : ${prefix}googletext nuy/cans/abiezz`)
					reply(ind.wait())
					nuy = await getBuffer(`https://videfikri.com/api/textmaker/gsuggest/?text1=${nuy}&text2=${ganz}&text3=${abiez}`)
					faa.sendMessage(from, nuy, image, {quoted: mek})
					await limitAdd(sender)
				break
		case 'tinyurl':
               faa.updatePresence(from, Presence.composing) 
		...
		case 'shrtco':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					exec(`wget ${anu.url} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						...
						faa.sendMessage(from, buffer, sticker, {quoted: mek})
						fs.unlinkSync(rano)
					})
					await limitAdd(sender)
					break
		...
		case 'randomhentai':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					reply(ind.wait())
                                        ...
                                        faa.sendMessage(from, buffer, image, {quoted: mek})
					await limitAdd(sender)
                                        break
		case 'cium':
                                        // Fix Bug Makanya Jagan Nganti Nama NuyFaa
		...
		case 'nulis':
				...
				faa.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'nulis2':
		...
		case 'bplogo':
				...
				buffer = await getBuffer(anu.result)
				faa.sendMessage(from, buffer, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'tahta':
         ...
         faa.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan Lupa Follow IG BOT : Instagram.com/mszpros*', quoted: mek})
         break
		case 'darkneon':
                case 'candlemug':
                case 'lovemsg':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                    ...
                    faa.sendMessage(from, buffer, image, {caption: 'Nih kak....\nFollow :\nhttp://www.īnstagram.com/thenay_xploit_', quoted: mek})
         break
         case 'wanted':
	if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  reply(ind.wait())
	  owgi = await faa.downloadAndSaveMediaMessage(ted)
	  tels = body.slice(7)
	  anu = await imgbb("08579d070df9a07cb1c2ee565aece767", owgi)
	  hehe = await getBuffer(`https://videfikri.com/api/textmaker/wanted/?urlgbr=${anu.display_url}&text1=Dicari&text2=${tels}`)
	 faa.sendMessage(from, hehe, image, {quoted:mek})
	} else {
	...
	break
		case 'hartatata3':
				...
				faa.sendMessage(from, bh, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'text2gif':
		...
		case 'attp':
				...
				faa.sendMessage(from, attp2, sticker, {quoted: mek})
				break
		case 'ttp':
		...
		case 'shota':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				    try {
						...
						faa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ingat! Cintai husbumu'})
					} catch (e) {
		  ...
		  case 'slowmo':
				...
				media = await faa.downloadAndSaveMediaMessage(encmedia)
				ran = getRandom('.mp3')
				exec(`ffmpeg -i ${media} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
				fs.unlinkSync(media)
				if (err) return reply('Error!')
				uhh = fs.readFileSync(ran)
				faa.sendMessage(from, uhh, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
				fs.unlinkSync(ran)
				})
				break
				...
				case 'tupai':
					...
					exec(`ffmpeg -i ${media} -filter:a "atempo=0.5,asetrate=65100" ${ran}`, (err, stderr, stdout) => {
						...
						faa.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
						fs.unlinkSync(ran)
					})
				break
				case 'gemok':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				if (args.length < 1) return reply('Teks nya mana ? NuyFaa?')
		...
		case 'randomkpop':
				...
				buffer = await getBuffer(anu.result)
				faa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih kpopnya kak...'})
				await limitAdd(sender)
				break
		case 'aesthetic':
				...
				faa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih aestheticnya kak...'})
				await limitAdd(sender)
				break
		case 'logoepep': 
				gatauda = body.slice(9)
				...
				faa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih hasilnya kak...'})
				await limitAdd(sender)
				break
		case 'logoepep2': 
				gatauda = body.slice(10)
        ...
        case 'joox':
				...
				faa.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
				faa.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${data.result.title}.mp3`, quoted: mek})
				await limitAdd(sender)
				break
		case 'play':   
				// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				faa.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.result.title}.mp3`, quoted: mek})
				await limitAdd(sender)
				break
		case 'play2':   
				// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				buffer = await getBuffer(anu.result.thumbnail)
				faa.sendMessage(from, buffer, image, {quoted: mek, caption: iinfomp3})
				laguu = await getBuffer(anu.result.url)
				faa.sendMessage(from, laguu , audio, {mimetype: 'audio/mp4', filename: `${anu.result.title}.mp3`, quoted: mek})
				await limitAdd(sender)
				break
		case 'limit':
				// Fix Bug Makanya Jagan Nganti Nama NuyFaa
		...
		case 'vinta':
				...
				faa.sendMessage(from, vintage, image, {caption: 'Nih kak ${vin}', quoted: mek})
				await limitAdd(sender)
				break
         case 'satucak':
				    try {
                 ...
                 if (!isRegistered) return reply( ind.noregis())
						...
						faa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih Kak udah Jadi'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(ind.wrongf())
					}
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break	
                case 'faktaunik': // Fixed By NuyFaaツ & FxSx
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
		...
		case 'infogempa':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					hasil = `*Potensi*\n${anu.potensi}\n*Lokasi*\n${anu.lokasi}\n*Magnitude*\n${anu.magnitude}\n*Koordinat*\n${anu.koordinat}\n*Kedalaman*\n${anu.kedalaman}\n*Waktu*\n${anu.waktu}\n*Map*\n${anu.map}`
					faa.sendMessage(from, buffer, image, {quoted: mek, caption: hasil})
					await limitAdd(sender)
					break
		case 'infocuaca':
					tels = body.slice(11)
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, hasil, text, {quoted: mek})
					await limitAdd(sender)
					break
		case 'kurs': 
                 if (!isRegistered) return reply( ind.noregis())
		...
		case 'github': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, buffer7, image, {quoted: mek, caption: capt})
					await limitAdd(sender)
					break
                case 'slap':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, pod, image, { quoted: mek, caption: '*Toxic*\n\n'+ ple })
					await limitAdd(sender)
					break
		case 'tampar':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
		...
		case 'setppbot':
					...
					media = await faa.downloadAndSaveMediaMessage(enmedia)
					await faa.updateProfilePicture(botNumber, media)
		...
		case 'brainly':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, teks, text, {quoted: mek, detectLinks: false})
					console.log(res)
					})
					await limitAdd(sender)
					break 
		case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buffer = await faa.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							faa.sendMessage(_.jid, buffer, image, {caption: `*「 BC GROUP 」*\n\nDari Grup : ${groupName}\nPengirim : wa.me/${(sender.split('@')[0])}\nPesan : ${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
		...
		case 'pinterest':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, pok, image, { quoted: mek, caption: `*PINTEREST*`})
					await limitAdd(sender)
					break 
           case 'glitchtext':
					...
					var gli = gh.split("/")[0];
					var tch = gh.split("/")[1];
					if (args.length < 1) return reply(`ã€Œâ—ã€Contoh : ${prefix}glitchtext Nuy/Cans`)
					reply(ind.wait())
           ...
           case 'imagegame':
		     ...
		     case 'fakta':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					faa.sendMessage(from, fakta, text, {quoted: mek })
					await limitAdd(sender) 
					break 
				case 'katabijak':
			        // Fix Bug Makanya Jagan Nganti Nama NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
					...
					faa.sendMessage(from, katabijak, text, {quoted: mek })
					await limitAdd(sender) 
					break
			case 'katailham':
			        // Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				kata = anu.result
				faa.sendMessage(from, kata, text, {quoted: mek })
				await limitAdd(sender)
				break
				case 'caklontong':
			        // Fix Bug Makanya Jagan Nganti Nama NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
					...
					setTimeout( () => {
					faa.sendMessage(from, '*тЮ╕ Jawaban :* '+anu.jawaban+ '\n\nтАв Penjelasan: *'+ anu.keterangan+'*', text, {quoted: mek }) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					faa.sendMessage(from, '_10 Detik lagiтАж_', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					faa.sendMessage(from, '_20 Detik lagi_тАж', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					faa.sendMessage(from, '_30 Detik lagi_тАж', text) // ur cods
					}, 2500) // 1000 = 1s,
					setTimeout( () => {
					faa.sendMessage(from, caklontong, text, {quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break 
		         case 'babi':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                 const gshizuka = await faa.getGroupMembersId(groupId)
                 let gmik = gshizuka[Math.floor(Math.random() * gshizuka.length)]
                 const mmkk = `YANG PALING BABI DISINI ADALAH @${gmik.replace(/@c.us/g, '')}`
                 faa.sendTextWithMentions(dari, mmkk, id)
                 break
				case 'tebakgambar':
			        // Fix Bug Makanya Jagan Nganti Nama NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
					...
					setTimeout( () => {
					faa.sendMessage(from, '*тЮ╕ Jawaban :* '+anu.jawaban, text, {quoted: mek }) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					faa.sendMessage(from, '_10 Detik lagiтАж_', text) // ur cods
					}, 20000) // 1000 = 1s,
					...
					setTimeout( () => {
					faa.sendMessage(from, bufferkkk, image, { caption: '_Jelaskan Apa Maksud Gambar Ini_', quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break  
		case 'spamcall':
			...
			faa.sendMessage(from, `${anu.result.logs}`, text, {quoted: mek })
			await limitAdd(sender) 
			break  
				case 'wiki':
			        // Fix Bug Makanya Jagan Nganti Nama NuyFaa
                ...
                if (anu.error) return reply(anu.error)
                wikii = `${anu.result}`
                faa.sendMessage(from, wikii, text, {quoted: mek })
                await limitAdd(sender) 
                break
               case 'asupan':
			        // Fix Bug Makanya Jagan Nganti Nama NuyFaa
                ...
                faa.sendMessage(from, asup, video, {mimetype: 'video/mp4', filename: `asupan_bangsa.mp4`, quoted: mek, caption: 'Asupannya Tuan:v'})
                await limitAdd(sender) 
                break
               case 'ytmp4':  // Awas Bug Xixixi By NuyFaa
	if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
	  reply(ind.wait())
	  ...
	  teks = `_â‰»TITLE_:${anu.result.judul}\n\n_Mohon Tunggu Zayang video sedang di kirim^^_`
	  faa.sendMessage(from, buf, image, { quoted: mek, caption: teks })
	  faa.sendMessage(from, bufq, video, { caption: 'nih' }) 
	await limitAdd(sender)
	 break 
			case 'ytmp3': // Awas Bug Xixixi By NuyFaa
					if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					faa.sendMessage(from, buffing, image, {quoted: mek, caption: yta})
					faa.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', filename: `${anu.result.judul}.mp3`, quoted: mek, caption: 'Nih Gan'})
					await limitAdd(sender) 
					break 
			 case 'nsfwloli':
				    try {
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
						...
						faa.sendMessage(from, buffer, image, {quoted: shizuka, caption: 'Jangan jadiin bahan buat comli om'})
					    } catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					    }
		...
		case 'alay2': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					reply(data.result)
				        await limitAdd(sender)
					break
		case 'fml':	// Fixed By NuyFaaツ & FxSx
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
		...
		case 'heroml':  
				...
				faa.sendMessage(from, buffer7, image, {quoted: mek, caption: teks})
				await limitAdd(sender)
				break
				case 'igvideo': 
                 if (!isRegistered) return reply( ind.noregis())
				...
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
					...
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
                    faa.sendMessage(from, buffer, video, )
				    break
				case 'igpost': 
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
					...
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
                    faa.sendMessage(from, buffer, image, )
				    break
				case 'igtv': 
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
                ...
                case 'kickall':
					...
					faa.groupRemove(from, members_id)
					break*/
		case 'setreply':
					if (!isOwner) return reply(ind.ownerb())
					faa.updatePresence(from, Presence.composing) 
		...
		case 'grouplist':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					faa.sendMessage(from, teks.trim(), extendedText, {quoted: mek})
					break
		//daftar 
		case 'daftar':
                			...
                			if (umurUser < 12) return reply(`your age is too young minimum 12 years`)
                					try {
								ppimg = await faa.getProfilePicture(`${sender.split('@')[0]}@c.us`)
								} catch {
								ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
							}
                					veri = sender
                    			...
                    			await faa.sendMessage(from, ppimg, image, {quoted: mek, caption: ind.registered(namaUser, umurUser, serialUser, time, sender)})
                    			addATM(sender)
		...
		case 'bisakah':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, 'Pertanyaan : *'+bisakah+'*\n\nJawaban : '+ keh, text, { quoted: mek })
					await limitAdd(sender)
					break
		case 'kapankah':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, 'Pertanyaan : *'+kapankah+'*\n\nJawaban : '+ koh, text, { quoted: mek })
					await limitAdd(sender)
					break
		case 'apakah':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, 'Pertanyaan : *'+apakah+'*\n\nJawaban : '+ kah, text, { quoted: mek })
					await limitAdd(sender)
					break
		case 'bagaimanakah':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, 'Pertanyaan : *'+bagaimanakah+'*\n\nJawaban : '+ mana, text, { quoted: mek })
					await limitAdd(sender)
					break
		case 'rate':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, 'Pertanyaan : *'+rate+'*\n\nJawaban : '+ te+'%', text, { quoted: mek })
					await limitAdd(sender)
					break
                case 'sangecek':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, 'Pertanyaan : *'+sange+'*\n\nJawaban : '+ nge+'%', text, { quoted: mek })
					await limitAdd(sender)
					break
                case 'gaycek':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, 'Pertanyaan : *'+gayy+'*\n\nJawaban : '+ yag+'%', text, { quoted: mek })
					await limitAdd(sender)
					break
                case 'lesbicek':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, 'Pertanyaan : *'+lesbii+'*\n\nJawaban : '+ bi+'%', text, { quoted: mek })
					await limitAdd(sender)
					break
                case 'gantengcek':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, 'Pertanyaan : *'+ganteng+'*\n\nJawaban : '+ teng+'%', text, { quoted: mek })
					await limitAdd(sender)
					break
		case 'cantikcek':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, 'Pertanyaan : *'+cantik+'*\n\nJawaban : '+ tik+'%', text, { quoted: mek })
					await limitAdd(sender)
					break
		case 'watak':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, 'Pertanyaan : *'+watak+'*\n\nJawaban : '+ tak, text, { quoted: mek })
					await limitAdd(sender)
				        break
		case 'hobby':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, 'Pertanyaan : *'+hobby+'*\n\nJawaban : '+ by, text, { quoted: mek })
					await limitAdd(sender)
					break
		case 'speed5':
		case 'ping3':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
					await faa.sendMessage(from, `Pong!!!!\nSpeed: ${processTime(time, moment())} _Second_`)
					break
		case 'help': 
		case 'menu':
            // Awas Aja Di Hapus Tqs Too Nya Saya Bikin Malu 
                 if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
...
► Nama : NuyFaaツ
► WhatsApp : http://ẉa.me/6283815956151
...
📌NOTE :
Kasih Jeda Buat Bot, Misalkan Ga Keluar Perintahnya
Mungkin Eror/Pending/Paket0%/Tidur Ownernya Xixixi
Spam Call, Spam Command = *BLOCK + BANNED*` 

                    faa.sendMessage(from, wew, image, { quoted: mek, caption: faa })
					break
		case 'donasi':
		case 'donate':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
					faa.sendMessage(from, donasi(), text)
					break
		case 'menupicture':
		case 'menupict':
		case 'pictmenu':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					wew = fs.readFileSync(`./src/logo.jpg`)
					faa.sendMessage(from, wew, image, { quoted: mek, caption: randompict(prefix)})
					break
		case 'menuowner':
		case 'ownermenu':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					wew = fs.readFileSync(`./src/logo.jpg`)
					faa.sendMessage(from, wew, image, { quoted: mek, caption: developer(prefix)})
					break
		case 'bahasa':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					faa.sendMessage(from, bahasa(), text)
					await limitAdd(sender)
					break
		case 'kodenegara':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					faa.sendMessage(from, negara(), text)
					await limitAdd(sender)
					break
		case 'del':
		case 'd':
		case 'delete':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					faa.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					await limitAdd(sender)
					break
		case 'level':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
					...
					faa.sendMessage(from, resul, text, { quoted: mek})
					.catch(async (err) => {
					console.error(err)
					await reply(`Error!\n${err}`)
					})
		...
		case 'say':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        ...
                                        saying = teks
                                        faa.sendMessage(from, saying, text)
					await limitAdd(sender)
                                        break
		case 'info':
					me = faa.user
					uptime = process.uptime()
					teks = `*INFO OWNER*\n*Owner bot* : NuyFaaツ\n*No Owner* : http://ẉa.me/6283815956151\n*Ig owner* : http://www.īnstagram.com/thenay_xploit_\n━━━━━━━━━━━━━━━━━━━━\n*INFO BOT*\n*Nama bot* : ${me.name}\n*Nomor bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total block contact* : ${blocked.length}\n*The bot is active on* : ${kyun(uptime)}\n*Ketik* : ${prefix}lpr _Untuk melaporkan admin bot melalui bot_\n*Ketik* : ${prefix}owner untuk menghubungi admin bot, jika ada bug kasih tau ^_*`
					buffer = await getBuffer(me.imgUrl)
					faa.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
		case 'snk':
					me = faa.user
					uptime = process.uptime()
		...
		case 'lpr':
					...
					if (pesan.length > 300) return faa.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
					...
					faa.sendMessage('6283815956151@s.whatsapp.net', options, text, {quoted: mek})
					reply('Masalah telah di laporkan ke owner MANCA-BOT, Laporan palsu atau main² tidak akan ditanggapi.')
					break
		case 'request':
					const cfrr = body.slice(8)
					if (cfrr.length > 300) return faa.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
					var nomor = mek.participant
		...
		case 'blocklist': 
					...
					faa.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": blocked}})
					break
                case 'hidetag':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					var group = await faa.groupMetadata(from)
					...
					faa.sendMessage(from, options, text)
					await limitAdd(sender)
					break
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                                ...
                                const tagzz = {
                                        ...
                                        faa.sendMessage(from, tagzz, text, {quoted: mek})
					await limitAdd(sender)
                                        break
                                        case 'profile':
					...
					ppimg = await faa.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
					...
					faa.sendMessage(from, buffer, image, {quoted: mek, caption: profile})
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					buffer = await getBuffer(anu.result)
					faa.sendMessage(from, buffer, image, {caption: 'Nih kak', quoted: mek})
					await limitAdd(sender)
					break
                case 'truth':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
					await limitAdd(sender)
					break
		case 'dare':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, tod, image, { quoted: mek, caption: '*Dare*\n\n'+ der })
					await limitAdd(sender)
					break
		case 'quoteskehidupan':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, crot, image, { quoted: mek, caption: '*Quotes Kehidupan*\n\n'+ quo })
					await limitAdd(sender)
					break
		case 'quotesislami':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, islam, image, { quoted: mek, caption: '*Quotes Islami*\n\n'+ isl })
					await limitAdd(sender)
					break	
		case 'quotesnasehat':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, nase, image, { quoted: mek, caption: '*Quotes Nasehat*\n\n'+ nsh })
					await limitAdd(sender)
					break	
		case 'quotescinta':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, cta, image, { quoted: mek, caption: '*Quotes Cinta*\n\n'+ cin })
					await limitAdd(sender)
					break
		case 'quotesmotivasi':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, vass, image, { quoted: mek, caption: '*Motivasi*\n\n'+ cin })
					await limitAdd(sender)
					break
                case 'fotocewek':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
	                                const cangti = ['https://i.ibb.co/1T1DCz7/cewek-thailand-20200325-007-non-fotografer-kly.jpg','https://i.ibb.co/FsJ6jjs/kucing2.jpg','https://i.ibb.co/vvkdS7n/kucing3.jpg','https://i.ibb.co/1QHWxts/kucing4.jpg','https://i.ibb.co/JQmRz4n/kucing5.jpg','https://i.ibb.co/tBwrFkG/kucing6.jpg','https://i.ibb.co/dp0YhYm/kucing7.jpg','https://i.ibb.co/R03smZT/kucing8.jpg','https://i.ibb.co/17tw0dp/kucing9.jpg','https://i.ibb.co/7XdGGqc/kucing10.jpg','https://i.ibb.co/XL9PZxg/kucing11.jpg','https://i.ibb.co/gyjvXWN/kucing12.jpg','https://i.ibb.co/R4gg4wH/kucing13.jpg','https://i.ibb.co/PmLYtFm/kucing14.jpg','https://i.ibb.co/XbSDh47/kucing15.jpg','https://i.ibb.co/kSXNJzt/kucing16.jpg']
                                        ...
                                        faa.sendMessage(from, cangtip, image, {quote: mek})
					await limitAdd(sender)
                                        break
                /*case 'kucing':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
	                                const kucings = ['https://i.ibb.co/vvmp82w/kucing1.jpg']
                                        ...
                                        faa.sendMessage(from, kucigz, image, {quote: mek})
					await limitAdd(sender)
                                        break*/
		case 'ssweb':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
		...
		case 'waifu':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, hasil, image, {quoted: mek})
					await limitAdd(sender)
					break
		case 'loli2':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
		...
		case 'pokemon':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					pok = await getBuffer(nimek)
					faa.sendMessage(from, pok, image, { quoted: mek })
					await limitAdd(sender)
					break
				case 'freefireimg':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
		...
		case 'kelinci':
				...
				case 'waifu':
				  	...
				  	buf = await getBuffer(anu.image)
				   	...
				   	faa.sendMessage(from, buf, image, { quoted: mek, caption: `${texs}`})
				        break
					case 'animeboy':
				...
				case 'loli':
					...
					break
                		...
                		case 'dewabatch':
                    			...
                    			faa.sendMessage(from, thum, image, {quoted: mek, caption: `${anu.result}`})
                 			break
			   case 'animequotes':
		...
		case 'map':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					buffer = await getBuffer(anu.gambar)
					faa.sendMessage(from, buffer, image, {quoted: mek, caption: `${body.slice(5)}`})
					await limitAdd(sender)
					break
                case 'kbbi':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
		...
		case 'quran':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					faa.sendMessage(from, quran, text, {quoted: mek})
					await limitAdd(sender)
					break
		case 'jadwalsholat':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
				...
				if (args.length < 1) return reply('kota nya mana kak?')
					...
					faa.sendMessage(from, sholat, text, {quoted: mek})
					await limitAdd(sender)
					break
                case 'neonime':
				        // Fix Bug Makanya Jagan Nganti Nama NuyFaa
		...
		case 'ocr': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						const media = await faa.downloadAndSaveMediaMessage(encmedia)
						reply(ind.wait())
		...
		case 'tts':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				if (args.length < 1) return faa.sendMessage(from, 'Diperlukan kode bahasa kak!!', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return faa.sendMessage(from, 'Mana teks yang mau di jadiin suara? suara setan kah?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 500
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							buffer = fs.readFileSync(rano)
							if (err) return reply(ind.stikga())
							faa.sendMessage(from, buffer, audio, {quoted: mek, ptt:true})
							fs.unlinkSync(rano)
						})
					})
					await limitAdd(sender)
		...
		case 'tiktokstalk':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				try {
						if (args.length < 1) return faa.sendMessage(from, '*Username mana kak?', text, {quoted: mek})
				...
				linkgc = await faa.groupInviteCode (from)
				yeh = `https://chat.whatsapp.com/${linkgc}\n\nlink Group *${groupName}*`
				faa.sendMessage(from, yeh, text, {quoted: mek})
				await limitAdd(sender)
				break
		case 'sayang':
					faa.sendMessage(from, 'Apa sayangku? 🥰',MessageType.text, { quoted: mek} )
					break
		case 'gelud':
					faa.sendMessage(from, 'Kuylah, dapat duit aku..',MessageType.text, { quoted: mek} )
					break
		case 'bot':
					faa.sendMessage(from, 'Iya kak? \nJangan lupa daftar ya kak ^_^"\nJika sudah silahkan berarti sudah terdaftar kak ^_^"',MessageType.text, { quoted: mek} )
					break
		case 'hargaprem':
		case 'hargapremium':
					faa.sendMessage(from, 'Level 100 = Free 3 day (premium)\n10k = 30day (premium)',MessageType.text, { quoted: mek} )
					break
		case 'buypremium':
					faa.sendMessage(from, 'Ingin membeli premium?\nHarap hubungi kami : wa.me/6283815956151',MessageType.text, { quoted: mek} )
					break
		case 'buypremunlimited':
					
					faa.sendMessage(from, 'Selamat kamu sudah berlangganan selamanya 🎉',MessageType.text, { quoted: mek} )
					break
		case 'buyprem30day':
					
					faa.sendMessage(from, 'Selamat kamu sudah berlangganan 30 hari',MessageType.text, { quoted: mek} )
					break
		case 'buyprem7day':
					
					faa.sendMessage(from, 'Selamat kamu sudah berlangganan 7 hari',MessageType.text, { quoted: mek} )
					break
		case 'test':
					faa.sendMessage(from, 'Active',MessageType.text, { quoted: mek} )
					break
		case 'ping':
					faa.sendMessage(from, 'Active',MessageType.text, { quoted: mek} )
					break
		case 'assalamualaikum':
					faa.sendMessage(from, 'Waalaikumusalam',MessageType.text, { quoted: mek} )
					break
		case 'kontol':
					faa.sendMessage(from, 'Gunakan bahasa yang benar\nAnda akan kami banned!\nHubungi kami : wa.me/6283815956151',MessageType.text, { quoted: mek} )
					break
		...
		case 'wame':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
  					...
  					faa.sendMessage(from, options, text, { quoted: mek } )
  					break
  					if (data.error) return reply(data.error)
  					reply(data.result)
					await limitAdd(sender)
		...
		case 'clearall':
					if (!isOwner) return reply(ind.ownerb())
					anu = await faa.chats.all()
					faa.setMaxListeners(25)
					for (let _ of anu) {
						faa.deleteChat(_.jid)
					}
					reply(ind.clears())
					break
		case 'blockk':
				 ...
				 faa.chatRead (from)
					if (!isGroup) return reply(ind.groupo())
				if (!isAdmin) return reply('*Only Admin bot*')
					faa.blockUser (`${body.slice(8)}@c.us`, "add")
					faa.sendMessage(from, `*Perintah Diterima, Memblokir* ${body.slice(7)}@c.us`, text)
					break
		case 'unblockk':
					if (!isGroup) return reply(ind.groupo())
				if (!isAdmin) return reply('*Only Admin bot*')
				    faa.blockUser (`${body.slice(10)}@c.us`, "remove")
					faa.sendMessage(from, `*Perintah Diterima, Membuka Blockir* ${body.slice(9)}@c.us`, text)
					break
		case 'block':
				 faa.updatePresence(from, Presence.composing) 
				 faa.chatRead (from)
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
					faa.blockUser (`${body.slice(7)}@c.us`, "add")
					faa.sendMessage(from, `*Perintah Diterima, Memblokir* ${body.slice(7)}@c.us`, text)
					break
		case 'unblock':
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
				    faa.blockUser (`${body.slice(9)}@c.us`, "remove")
					faa.sendMessage(from, `*Perintah Diterima, Membuka Blockir* ${body.slice(9)}@c.us`, text)
					break
		case 'leave':
					// Fix Bug Makanya Jagan Nganti Nama NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
					...
					setTimeout( () => {
					faa.updatePresence(from, Presence.composing) 
					faa.sendMessage(from, 'Bye cuk🗣', text) // ur cods
					}, 0)
					break
		case 'bc': 
					if (!isOwner) return reply(ind.ownerb()) 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						buff = await faa.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							faa.sendMessage(_.jid, buff, image, {caption: `*「 PESAN BROADCAST 」*\n\n${body.slice(4)}`})
						}
		...
		case 'setpp': 
					...
					if (!isGroupAdmins) return reply(ind.admin())
                   			if (!isBotGroupAdmins) return reply(ind.badmin())
					media = await faa.downloadAndSaveMediaMessage(mek)
					await faa.updateProfilePicture (from, media)
		...
		case 'group':
					...
					if (args[0] === 'buka') {
						...
						faa.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`*BERHASIL MENUTUP GROUP*`)
						faa.groupSettingChange(from, GroupSettingChange.messageSend, true)
					}
					break      
            case 'admin':
            case 'owner':
            case 'creator':
            case 'developer':
                  faa.sendMessage(from, {displayname: "Jeff", vcard: vcard}, MessageType.contact, { quoted: mek})
                  faa.sendMessage(from, 'Tuh nomer owner ku >_<, jangan spam atau ku block kamu',MessageType.text, { quoted: mek} )
					break    
           case 'setname':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					faa.groupUpdateSubject(from, `${body.slice(9)}`)
					faa.sendMessage(from, 'Succes, Ganti Nama Grup', text, {quoted: mek})
					break
                case 'setdesc':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					faa.groupUpdateDescription(from, `${body.slice(9)}`)
					faa.sendMessage(from, 'Succes, Ganti Deskripsi Grup', text, {quoted: mek})
					break
           case 'demote':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					...
					if (mentioned.length > 1) {
						...
						faa.groupDemoteAdmin(from, mentioned)
					} else {
			...
			case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(teks, mentioned, true)
						faa.groupMakeAdmin(from, mentioned)
					} else {
		...
		case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(teks, mentioned, true)
						faa.groupRemove(from, mentioned)
					} else {
		...
		case 'toimg':
				...
				if (!isQuotedSticker) return reply('*Reply/Tag sticker!*')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						faa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih Kak'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
                 case 'simi':
if (isBanned) return reply('```Awok Udah Di Bened Sama Owner```')
					if (args.length < 1) return reply(`Hai ${pushname}`)
					teks = body.slice(4)
					try { 
					anu = await fetchJson(`https://simsumi.herokuapp.com/api?text=BotWea`, {method: 'get'})
		...
		case 'welcome':
                 ...
                 case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						faa.sendMessage(from,`Perhatian kepada seluruh member anti link group aktif apabila anda mengirim link group anda akan di kick dari group`, text)
					} else if (Number(args[0]) === 0) {
		...
		case 'clone':
					...
					try {
						pp = await faa.getProfilePicture(id)
						buffer = await getBuffer(pp)
						faa.updateProfilePicture(botNumber, buffer)
				...
				case 'wait':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await wait(media).then(res => {
							faa.sendMessage(from, res.video, video, {quoted: mek, caption: res.teks.trim()})
						}).catch(err => {
							reply(err)
						})
					} else {
				...
				if (body.startsWith(`${prefix}${command}`)) {
                  reply(`*maaf kak, ᴄommand ${prefix}${command} tidak tedaftar di MANCA BOT*\nKetik : ${prefix}menu`)
               }
